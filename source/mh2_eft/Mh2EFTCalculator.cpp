    ...
    : p(p_), msq2(msq2_)
{
    ...
    if (std::abs(p.g1) < eps) {
        setCorrectionFlag(EFTCouplingOrders::G12G22, 0);
        setCorrectionFlag(EFTCouplingOrders::G12YB2, 0);
        setCorrectionFlag(EFTCouplingOrders::G14, 0);
        setCorrectionFlag(EFTCouplingOrders::G12YB2, 0);
        setCorrectionFlag(EFTCouplingOrders::G12YTAU2, 0);
        setCorrectionFlag(EFTCouplingOrders::G12YT2, 0);
    }

    if (std::abs(p.g2) < eps) {
        setCorrectionFlag(EFTCouplingOrders::G24, 0);
        setCorrectionFlag(EFTCouplingOrders::G22YB2, 0);
        setCorrectionFlag(EFTCouplingOrders::G22YTAU2, 0);
        setCorrectionFlag(EFTCouplingOrders::G22YT2, 0);
    }

    if (std::abs(p.Mt) < eps) {
        setCorrectionFlag(EFTCouplingOrders::G12YT2  , 0);
        setCorrectionFlag(EFTCouplingOrders::G22YT2  , 0);
        setCorrectionFlag(EFTCouplingOrders::YT4     , 0);
        setCorrectionFlag(EFTCouplingOrders::G32YT4  , 0);
        setCorrectionFlag(EFTCouplingOrders::YT6     , 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU2YT4, 0);
        setCorrectionFlag(EFTCouplingOrders::YT2YB4  , 0);
        setCorrectionFlag(EFTCouplingOrders::YB2YT4  , 0);
    }

    if (std::abs(p.Mb) < eps) {
        setCorrectionFlag(EFTCouplingOrders::G12YB2  , 0);
        setCorrectionFlag(EFTCouplingOrders::G22YB2  , 0);
        setCorrectionFlag(EFTCouplingOrders::YB4     , 0);
        setCorrectionFlag(EFTCouplingOrders::G32YB4  , 0);
        setCorrectionFlag(EFTCouplingOrders::YB6     , 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU2YB4, 0);
        setCorrectionFlag(EFTCouplingOrders::YT2YB4  , 0);
        setCorrectionFlag(EFTCouplingOrders::YB2YT4  , 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU4YB2, 0);
        setCorrectionFlag(EFTCouplingOrders::YT6     , 0);
    }

    if (std::abs(p.Mtau) < eps) {
        setCorrectionFlag(EFTCouplingOrders::G12YTAU2, 1);
        setCorrectionFlag(EFTCouplingOrders::G22YTAU2, 1);
        setCorrectionFlag(EFTCouplingOrders::YTAU4   , 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU2YB4, 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU2YT4, 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU6   , 0);
        setCorrectionFlag(EFTCouplingOrders::YTAU4YB2, 0);
    }
...
void Mh2EFTCalculator::setCorrectionFlag(EFTCouplingOrders::EFTCouplingOrders order, int flag)
{
    if (flag < 0 || flag > 1)
...
double Mh2EFTCalculator::getDeltaMh2EFT1Loop(int omitSMLogs, int omitMSSMLogs) const
{
    ...
    // corrections to Mh2
    const double dmh2g12g22 = isNaN(orders.at(EFTCouplingOrders::G12G22) * (
    ...
    const double dmh2g14 = isNaN(orders.at(EFTCouplingOrders::G14) * (
                                 ...
                                 "dmh2g14");
    const double dmh2g24 = isNaN(orders.at(EFTCouplingOrders::G24) * (
                                 ...
                                 "dmh2g24");
    const double dmh2g12yb2 = isNaN(orders.at(EFTCouplingOrders::G12YB2) * (((
                                        10 * dlambdayb2g12 - 3 * (3 * B00DR - 2 * dvyb2 + 3 * lmMt) * pow2(c2beta))
                                    * pow2(cbeta) * v2) / 20.),
                                    "dmh2g12yb2");
    const double dmh2g22yb2 = isNaN(orders.at(EFTCouplingOrders::G22YB2) * (((
                                        2 * dlambdayb2g22 + (-3 * B00DR + 2 * dvyb2 - 3 * lmMt) * pow2(c2beta)) * pow2(
                                        cbeta) * v2) / 4.),
                                    "dmh2g22yb2");
    const double dmh2yb4 = isNaN(orders.at(EFTCouplingOrders::YB4) * (((12 * B00DR
                                 + dlambdayb4 + 12 * lmMt) * v2 * pow4(cbeta)) / 2.),
                                 "dmh2yb4");
    const double dmh2g12yt2 = isNaN(orders.at(EFTCouplingOrders::G12YT2) * (((10
                                    * dlambdayt2g12 + (6 + 6 * dvyt2 - 9 * lmMt) * pow2(c2beta)) * pow2(sbeta) * v2)
                                    / 20.),
                                    "dmh2g12yt2");
    const double dmh2g22yt2 = isNaN(orders.at(EFTCouplingOrders::G22YT2) * (((2
                                    * dlambdayt2g22 + (2 + 2 * dvyt2 - 3 * lmMt) * pow2(c2beta)) * pow2(sbeta)
                                    * v2) / 4.),
                                    "dmh2g22yt2");
    const double dmh2yt4 = isNaN(orders.at(EFTCouplingOrders::YT4) * (pref_at * (12 * lmMt +
                                 thresholdCalculator.getThresholdCorrection(ThresholdCouplingOrders::LAMBDA_AT,
                                         RenSchemes::DRBARPRIME, omitMSSMLogs))),
                                 "dmh2yt4");
    const double dmh2g12ytau2 = isNaN(orders.at(EFTCouplingOrders::G12YTAU2) * (-((-10
                                      * dlambdaytau2g12 + 3 * B00DR * pow2(c2beta) + 3 * (-2 * dvytau2 + lmMt) *
                                      pow2(c2beta)) * pow2(cbeta) * v2) / 20.),
                                      "dmh2g12ytau2");
    const double dmh2g22ytau2 = isNaN(orders.at(EFTCouplingOrders::G22YTAU2) * (-((-2
                                      * dlambdaytau2g22 + B00DR * pow2(c2beta) + (-2 * dvytau2 + lmMt) * pow2(
                                          c2beta)) * pow2(cbeta) * v2) / 4.),
                                      "dmh2g22ytau2");
    const double dmh2ytau4 = isNaN(orders.at(EFTCouplingOrders::YTAU4) * (((4 * B00DR
...
double Mh2EFTCalculator::getDeltaMh2EFT2Loop(int omitSMLogs, int omitMSSMLogs) const
{
    ...
    // Threshold corrections
    const double dytas = CALC_IF(orders.at(EFTCouplingOrders::G32YT4),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::YT_AS, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayb4g32 = CALC_IF(orders.at(EFTCouplingOrders::G32YB4),
                                         thresholdCalculator.getThresholdCorrection(
                                                 ThresholdCouplingOrders::LAMBDA_YB4_G32, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayb4 =
        CALC_IF(orders.at(EFTCouplingOrders::YB6) || orders.at(EFTCouplingOrders::YTAU2YB4) ||
                orders.at(EFTCouplingOrders::YT2YB4) || orders.at(EFTCouplingOrders::G32YB4),
                thresholdCalculator.getThresholdCorrection(
                    ThresholdCouplingOrders::LAMBDA_YB4, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayb6 = CALC_IF(orders.at(EFTCouplingOrders::YB6),
                                      thresholdCalculator.getThresholdCorrection(
                                          ThresholdCouplingOrders::LAMBDA_YB6, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayt4 =
        CALC_IF(orders.at(EFTCouplingOrders::YT6) || orders.at(EFTCouplingOrders::YB2YT4),
                thresholdCalculator.getThresholdCorrection(
                    ThresholdCouplingOrders::LAMBDA_AT, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dytyt = CALC_IF(orders.at(EFTCouplingOrders::YT6),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::YT_YT, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayt6 = CALC_IF(orders.at(EFTCouplingOrders::YT6),
                                      thresholdCalculator.getThresholdCorrection(
                                          ThresholdCouplingOrders::LAMBDA_YT6, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dvyt2 = CALC_IF(orders.at(EFTCouplingOrders::YT6) || orders.at(EFTCouplingOrders::YT2YB4),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::VEV_YT2, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dytauytau = CALC_IF(orders.at(EFTCouplingOrders::YTAU6),
                                     thresholdCalculator.getThresholdCorrection(
                                         ThresholdCouplingOrders::YTAU_YTAU, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdaytau4 =
        CALC_IF(orders.at(EFTCouplingOrders::YTAU4YB2) || orders.at(EFTCouplingOrders::YTAU6),
                thresholdCalculator.getThresholdCorrection(
                    ThresholdCouplingOrders::LAMBDA_YTAU4, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdaytau6 = CALC_IF(orders.at(EFTCouplingOrders::YTAU6),
                                        thresholdCalculator.getThresholdCorrection(
                                            ThresholdCouplingOrders::LAMBDA_YTAU6, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayt2yb4 = CALC_IF(orders.at(EFTCouplingOrders::YT2YB4),
                                         thresholdCalculator.getThresholdCorrection(
                                                 ThresholdCouplingOrders::LAMBDA_YT2_YB4, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayt4yb2 = CALC_IF(orders.at(EFTCouplingOrders::YB2YT4),
                                         thresholdCalculator.getThresholdCorrection(
                                                 ThresholdCouplingOrders::LAMBDA_YT4_YB2, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dytyb = CALC_IF(orders.at(EFTCouplingOrders::YB2YT4),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::YT_YB, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dvytau2 =
        CALC_IF(orders.at(EFTCouplingOrders::YTAU2YB4) || orders.at(EFTCouplingOrders::YTAU6),
                thresholdCalculator.getThresholdCorrection(
                    ThresholdCouplingOrders::VEV_YTAU2, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dvyb2 =
        CALC_IF(orders.at(EFTCouplingOrders::YB6) || orders.at(EFTCouplingOrders::YTAU4YB2) ||
                orders.at(EFTCouplingOrders::YB2YT4),
                thresholdCalculator.getThresholdCorrection(
                    ThresholdCouplingOrders::VEV_YB2, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dytauyb = CALC_IF(orders.at(EFTCouplingOrders::YTAU4YB2),
                                   thresholdCalculator.getThresholdCorrection(
                                       ThresholdCouplingOrders::YTAU_YB, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayb4ytau2 = CALC_IF(orders.at(EFTCouplingOrders::YTAU2YB4),
                                           thresholdCalculator.getThresholdCorrection(
                                                   ThresholdCouplingOrders::LAMBDA_YTAU2_YB4, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dlambdayb2ytau4 = CALC_IF(orders.at(EFTCouplingOrders::YTAU4YB2),
                                           thresholdCalculator.getThresholdCorrection(
                                                   ThresholdCouplingOrders::LAMBDA_YTAU4_YB2, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dybyt = CALC_IF(orders.at(EFTCouplingOrders::YT2YB4),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::YB_YT, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dybas = CALC_IF(orders.at(EFTCouplingOrders::G32YB4),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::YB_AS, RenSchemes::DRBARPRIME, omitMSSMLogs));
    const double dybyb = CALC_IF(orders.at(EFTCouplingOrders::YB6),
                                 thresholdCalculator.getThresholdCorrection(
                                     ThresholdCouplingOrders::YB_YB, RenSchemes::DRBARPRIME, omitMSSMLogs));

    // Corrections to Mh
    const double dmh2yt4g32 = isNaN(orders.at(EFTCouplingOrders::G32YT4) * (pref * (96 * pow2(lmMt)
    ...
    const double dmh2yb4g32 =
        isNaN(orders.at(EFTCouplingOrders::G32YB4) * ((dlambdayb4g32
                + 16 * (3 * B00DR * (dybas + 16 * lmMt) + lmMt * (-2 + 3 * dybas
                        + 24 * lmMt))) * v2 * pow4(cbeta)) / 2.,
              "dmh2yb4g32");
    const double dmh2yb6 =
        isNaN(orders.at(EFTCouplingOrders::YB6) * (-((-144 * dybyb * (B00DR
              ...
              "dmh2yb6");
    const double dmh2yt6 = isNaN(orders.at(EFTCouplingOrders::YT6) * ((v2 * (4 * dytyt * (
                                 ...
                                 * pow2(sbeta)) * pow4(sbeta)) / 2.),
                                 "dmh2yt6");
    const double dmh2yb4ytau2 = isNaN(orders.at(EFTCouplingOrders::YTAU2YB4) * (((
                                          dlambdayb4ytau2 - 96 * B00DR * lmMt - dlambdayb4 * (-1 + B00DR + lmMt) + 2
                                          * dvytau2 * (-12 + 12 * B00DR + dlambdayb4 + 12 * lmMt) - 48 * pow2(lmMt)) * v2
                                      * pow6(cbeta)) / 2.),
                                      "dmh2yb4ytau2");
    const double dmh2yb2ytau4 = isNaN(orders.at(EFTCouplingOrders::YTAU4YB2) * (((4 * dytauyb
                                      * (4 * B00DR + dlambdaytau4 + 4 * lmMt) + pow2(cbeta) * (dlambdayb2ytau4 - 8 * dvyb2
                                      ...
                                      "dmh2yb2ytau4");
    const double dmh2ytau6 = isNaN(orders.at(EFTCouplingOrders::YTAU6) * (-((-12 * dytauytau
    ...
    const double dmh2yt2yb4 =
        isNaN(orders.at(EFTCouplingOrders::YT2YB4) * (((48 * dybyt * lmMt
              ...
              "dmh2yt2yb4");
    const double dmh2yt4yb2 = isNaN(orders.at(EFTCouplingOrders::YB2YT4) * (((4 * dytyb * (-6
